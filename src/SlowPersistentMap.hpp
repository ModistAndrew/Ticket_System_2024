...
#ifndef TICKETSYSTEM2024_SLOW_PERSISTENT_MAP_HPP
#define TICKETSYSTEM2024_SLOW_PERSISTENT_MAP_HPP

#include "Util.hpp"
#include "FileStorage.hpp"
#include "SuperFileStorage.hpp"

template<typename T, int CACHE_SIZE = 0>
class SlowPersistentMap { //use T::index as key
  struct TreeNode;
  struct LeafNode;
  ...
  public:
    ...
    iterator(SlowPersistentMap *set, int leafPos, int pos) : set(set), leafPos(leafPos), pos(pos),
  ...
  public:
    ...
    bool insert(SlowPersistentMap *set, const T &val, TreeNode *parent, int parentPos) {
      if (isLeaf) {
    ...
    bool erase(SlowPersistentMap *set, const INDEX &val, TreeNode *parent, int parentPos) {
      if (isLeaf) {
    ...
    iterator find(SlowPersistentMap *set, const INDEX &val, int loc) {
      if (isLeaf) {
  ...
  struct TreeNode {
    ...
    iterator find(SlowPersistentMap *set, const INDEX &val, int loc) { //find first no less than val
      int p = upper_bound(index, index + size - 1, val) - index;
      return set->getPtr(children[p], false).find(set, val, children[p]);
    }

    bool insert(SlowPersistentMap *set, const T &val, TreeNode *parent, int pos) { //insert val into this node
    ...
    bool erase(SlowPersistentMap *set, const INDEX &val, TreeNode *parent, int pos) { //erase val from this node
      int p = upper_bound(index, index + size - 1, val) - index;
      NodePtr child = set->getPtr(children[p], true);
    ...
    void postInsert(SlowPersistentMap *set, TreeNode *parent, int pos) { //when size==SIZE
      TreeNode newNode;
      int half = size / 2;
      newNode.size = half;
      size = half;
    ...
    void postErase(SlowPersistentMap *set, TreeNode *parent, int pos) { //when size==SIZE/2-1
    ...
    void merge(SlowPersistentMap *set, TreeNode *sibling, TreeNode *parent, int pos) { //sibling is parent->children[pos+1]
      memcpy(index + size - 1, parent->index + pos, sizeof(INDEX));
  ...
  struct LeafNode {
    ...
    void merge(SlowPersistentMap *set, LeafNode *sibling, TreeNode *parent, int pos) { //sibling is parent->children[pos+1]
...
public:
  int length;
  explicit SlowPersistentMap(std::string file_name) : treeNodeStorage(-1, file_name + "_tree"),
